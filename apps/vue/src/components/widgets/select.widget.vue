...
function onChange(event: Event) {
	...
	loginContext?.setFormState(props.formId, props.config.id, (event.target as HTMLInputElement).value);
}
</script>
...
<template>
	<div>
		...
		<template v-else>
			...
			<select :id="config.id" :name="config.id" :readonly="disabled" :required="validator?.required" size="1" @change="onChange($event)">
				<template v-for="option in config.options" :key="option.label">
					<optgroup v-if="option.type === 'group'" :label="option.label">
						<option
							...
							:key="subOption.value"
							:value="subOption.value"
							:selected="subOption.value === loginContext?.formContexts.value[formId]?.[config.id]"
						>
							{{ subOption.label }}
						</option>
					</optgroup>
					<option v-else :value="option.value" :selected="option.value === loginContext?.formContexts.value[formId]?.[config.id]">
						{{ option.label }}
					</option>
				</template>
			</select>
