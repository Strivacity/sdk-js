		...
		@if (mode === 'redirect' || mode === 'popup') {
			...
			<section>
				...
				<sty-login
					...
					[lang]="language"
					[params]="params"
					(login)="onEmbeddedLogin()"
					(close)="onEmbeddedClose()"
					(error)="onEmbeddedError($event)"
				></sty-login>
...
export default class LoginPage implements OnInit {
	...
	protected readonly params = extraParams;
	...
	ngOnInit(): void {
		...
		if (this.mode === 'embedded') {
			...
			if (this.authService.sdk.options.serverSideSession) {
				globalThis.location.href = `/auth/login?${new URLSearchParams(extraSearchParams).toString()}`;
			} else {
				void this.authService.login(extraParams);
			}
		} else if (this.mode === 'popup') {
			try {
				this.authService.login(extraParams);
			} catch (error) {
