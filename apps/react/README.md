# Strivacity React example app

A React SPA demonstrating [`@strivacity/sdk-react`](../../packages/sdk-react) - it shows all four authentication modes (`redirect`, `popup`, `embedded`, `native`) side by side, route guarding, token refresh/revoke, and pairing with the [backend (BFF) app](../backend) for server-managed sessions.
...
- [`@strivacity/sdk-react` README](../../packages/sdk-react/README.md) - the SDK this app is built on, including its full configuration reference
