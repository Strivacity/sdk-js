...
export class DateWidgetComponent implements OnInit {
	...
	protected readonly fieldLengths = fieldLengths;
	protected readonly placeholders = placeholders;
	protected readonly format: Array<'year' | 'month' | 'day'> = ['year', 'month', 'day'];
	...
	private readonly yearSignal = signal('');
	private readonly monthSignal = signal('');
	private readonly daySignal = signal('');

	protected readonly fieldValues = computed(() => ({ year: this.yearSignal(), month: this.monthSignal(), day: this.daySignal() }));
	...
	protected onFieldsetChange(event: Event): void {
		if (this.disabled()) {
			return;
		}
