# Strivacity Preact example app

A Preact SPA demonstrating [`@strivacity/sdk-preact`](../../packages/sdk-preact) - it shows all four authentication modes (`redirect`, `popup`, `embedded`, `native`) side by side, route guarding, token refresh/revoke, and pairing with the [backend (BFF) app](../backend) for server-managed sessions.
...
- [`@strivacity/sdk-preact` README](../../packages/sdk-preact/README.md) - the SDK this app is built on, including its full configuration reference
...
- `src/components/auth` - one folder per mode (`redirect`, `popup`, `embedded`, `native`); `components/auth/index.tsx` picks the active one based on `VITE_MODE`
...
- `native/widgets/` - one Preact component per widget type (`InputWidget`, `PasswordWidget`, `SelectWidget`, `SubmitWidget`, `PasskeyLoginWidget`, ...), each responsible for rendering and updating a single field
