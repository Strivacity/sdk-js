...
export type NuxtServerSDKOptions<Storage extends NuxtServerStorage = NuxtServerStorage, StateStorage extends SDKStorage = SDKStorage> = SDKOptions<
	Storage,
	StateStorage
> & {
	/**
	 * Optional URI to redirect to after a successful login.
	 */
	...
	/**
	 * Encrypted cookie storage secret.
	 */
	secret: string;

	/**
	 * URL prefix for auth routes.
	 * @default '/auth'
	 */
	authUrlPrefix: string;
};

export type NuxtServerSDKInitConfig = SDKInitConfig &
...
export type NuxtServerSDK = {
	/**
	 * Retrieves the current session data from the encrypted cookie storage.
	 *
	 * @param {H3Event} [event] - The H3 event object containing the request and response.
	 ...
	 *
	 * @param {SessionData} session - The new session data to be stored.
	 * @param {H3Event} [event] - The H3 event object containing the request and response.
	 * @returns {Promise<void>} A promise that resolves when the session has been updated.
	 */
	updateSession(session: SessionData, event?: H3Event): Promise<void>;
	 ...
	 */
	refreshSession(event?: H3Event): Promise<SessionData>;
	 ...
	 */
	revokeSession(event?: H3Event): Promise<void>;
	 ...
	 */
	completeLogin(params: Record<string, string>, event?: H3Event): Promise<SessionData>;
