...
export type ElementProps<T extends HTMLElement, Extra = object> = JSX.HTMLAttributes<T> & Partial<Omit<T, keyof HTMLElement>> & Extra;
...
export type SDKContext<Flow extends RedirectFlow | PopupFlow | NativeFlow | EmbeddedFlow> = {
	...
	/**
	 * Whether the SDK is still loading. This can be used to show a loading spinner or similar UI while the SDK is initializing.
	 */
	...
	/**
	 * Whether the user is authenticated. This can be used to conditionally render UI based on the user's authentication state.
	 */
	 ...
	 */
	readonly idTokenClaims: Accessor<IdTokenClaims | null>;

	/**
	 * The access token, if available. This can be used to authenticate API requests or other interactions with the backend.
	 */
	readonly accessToken: Accessor<string | null>;

	/**
	 * The refresh token, if available. This can be used to obtain new access tokens when the current one expires.
	 */
	readonly refreshToken: Accessor<string | null>;

	/**
	 * Whether the access token has expired. This can be used to trigger a refresh of the access token or to log the user out.
	 */
	readonly accessTokenExpired: Accessor<boolean>;
	 ...
	 */
	readonly accessTokenExpirationDate: Accessor<number | null>;
...
export type StyAuthProviderProps = ParentProps<{
	/**
...
export type LoginContext = {
	 ...
	 */
	forms: Accessor<Record<string, Record<string, unknown>>>;

	/**
	 * Current validation and info messages, keyed by form ID then widget ID.
	 */
	messages: Accessor<Record<string, Record<string, NativeFlowMessage>>>;

	/**
	 * The current login flow state returned by the backend.
	 */
	state: Accessor<Partial<NativeFlowState>>;
