 ...
 */
export async function deriveKey(secret: string, salt: Uint8Array, context: string): Promise<CryptoKey> {
 ...
 */
export async function encrypt(plaintext: string, secret: string, context: string): Promise<string> {
...
/**
 * Decrypts a Base64URL-encoded payload produced by {@link encrypt}.
 ...
 */
export async function decrypt(encoded: string, secret: string, context: string): Promise<string | null> {
	try {
