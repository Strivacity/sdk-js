...
import type { SDKHttpClient } from '../types';
...
export const defaultOptions: Partial<SDKOptions> = {
	lazyLoad: false,
	autoRefresh: true,
	redirectForTokenExchange: false,
	scopes: ['openid'],
	responseType: 'code',
	responseMode: 'query',
 ...
 */
export function getSDKOptions<InitOptions extends SDKInitConfig = SDKInitConfig, Options extends SDKOptions = SDKOptions>(
	state: FlowState,
	options: InitOptions,
): Options {
	...
	options.httpClient ??= createHttpClient();
	...
	options.redirectForTokenExchange ??= defaultOptions.redirectForTokenExchange;
 ...
 *
 * @param byteLength - The number of random bytes to generate. Defaults to 16 bytes (32 hex characters).
...
export function decodeJwt<T = Record<string, unknown>>(token: string): T {
	...
	return JSON.parse(new TextDecoder().decode(Base64URL.decode(payload))) as T;
}
...
export async function verifyJwt<T = Record<string, unknown>>(token: string, jwksUri: string): Promise<T> {
	...
	const header = JSON.parse(new TextDecoder().decode(Base64URL.decode(encodedHeader))) as { kid?: string; alg?: string };
	...
	return JSON.parse(new TextDecoder().decode(Base64URL.decode(encodedPayload))) as T;
}
...
async function generateJwt(header: object, payload: object, privateKey?: CryptoKey): Promise<string> {
	const encodedHeader = Base64URL.encode(new TextEncoder().encode(JSON.stringify(header)).buffer);
	const encodedPayload = Base64URL.encode(new TextEncoder().encode(JSON.stringify(payload)).buffer);
	...
	if (privateKey) {
		...
		return `${encodedToken}.${Base64URL.encode(signature)}`;
	}
...
export async function buildAuthorizationUrl<URLHandlerParams extends ExtraRequestArgs = ExtraRequestArgs>({
	...
	const url = new URL((await options.getMetadata()).authorization_endpoint);
...
export async function exchangeCode({
	...
	const requestedScopes = options.scopes ?? ['openid'];
	const returnedScopes = updatedSession.scope ? updatedSession.scope.split(' ') : [];

	if (!requestedScopes.every((s) => returnedScopes.includes(s))) {
...
export async function fetchFlowEntry({
	...
	const language = uri.searchParams.get('language') || globalThis.navigator?.language || 'en-US';
 ...
 * @param {ExtraRequestArgs} options.loginParams - The parameters for the login request, including optional prompts and hints
 * @param {string} options.language - The language to use for the session
 * @param {string} [options.sdkMode='web'] - The SDK mode to include in the login request (default is 'web')
 * @param {SDKOptions} options.options - The SDK options containing client ID, redirect URI, storage, and other options
 ...
 */
export async function startLoginSession<URLHandlerParams extends ExtraRequestArgs = ExtraRequestArgs>({
	loginParams,
	language,
	sdkMode = 'web',
	options,
}: {
	loginParams: URLHandlerParams;
	language: string;
	sdkMode?: string;
	options: SDKOptions;
}): Promise<Record<string, string>> {
	const url = await buildAuthorizationUrl({ params: loginParams, options: options });

	url.searchParams.append('sdk', sdkMode);
	...
	if (uri.searchParams.has('code')) {
		...
		return (await options.callbackHandler(uri, options.responseMode)) as Record<string, string>;
	}

	if (uri.searchParams.has('error')) {
		throw new Error(`${uri.searchParams.get('error')}: ${uri.searchParams.get('error_description')}`);
	}
	...
	if (uri.searchParams.has('language')) {
		lang = uri.searchParams.get('language')!;
	}
	if (uri.searchParams.has('short_app_id')) {
		shortAppId = uri.searchParams.get('short_app_id')!;
	}

	const ret: Record<string, string> = { session_id: sessionId, language: lang };

	if (shortAppId) {
		ret.short_app_id = shortAppId;
	}
...
export async function submitLoginForm({
	...
	if (data.hostedUrl && !data.forms && !data.messages) {
