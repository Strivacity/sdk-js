...
export async function popupUrlHandler(
	...
	features.left = Math.max(0, Math.round(globalThis.window.screenX + (globalThis.window.outerWidth - (features.width as number)) / 2));

	if (features.height !== undefined) {
		features.top = Math.max(0, Math.round(globalThis.window.screenY + (globalThis.window.outerHeight - (features.height as number)) / 2));
	}
	...
	const data = await new Promise<Record<string, string>>((resolve, reject) => {
		const listener = (event: MessageEvent<Record<string, string>>) => {
			const sameOrigin = !params?.checkOrigin || event.origin === globalThis.window.location.origin;
