...
export type ExtraRequestArgs = {
	...
	/**
	 * The URL of the login endpoint that the SDK will use to initiate the login flow. This is typically used in server-side session scenarios to redirect the user to the login page.
	 *
	 * If not provided, the SDK will use the default login endpoint based on the configured issuer.
	 *
	 * @type {string | URL}
	 */
	loginSessionUri?: string;
};
...
export type SDKOptions<Storage extends SDKStorage = SDKStorage, StateStorage extends SDKStorage = SDKStorage> = {
	...
	/**
	 * Indicates whether the SDK should use server-side session management, which involves handling session data on the server rather than in the client.
	...
	/**
	 * Overrides the default session start handler for embedded and native flows.
	 * If provided, replaces the built-in `startSession` logic entirely.
	 *
	 * @param {Record<string, unknown>} params - The login parameters passed to the session start.
	 * @returns - A promise that resolves when the session start is complete.
	 */
	startSessionHandler?: (params: Record<string, unknown>) => Promise<void>;

	/**
	 * Overrides the default session finalize handler for embedded and native flows.
	 * If provided, replaces the built-in `finalizeSession` logic entirely.
	 *
	 * @param {string | URL} url - The URL used to finalize the session.
	 * @returns - A promise that resolves when the session finalize is complete.
	 */
	finalizeSessionHandler?: (url: string | URL) => Promise<void>;
};
