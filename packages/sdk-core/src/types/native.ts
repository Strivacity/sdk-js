...
export type LoginFlowState = {
	 ...
	 *
	 * @type {Array<FormWidget> | undefined}
	 */
	 ...
	 *
	 * @type {Record<string, Record<string, LoginFlowMessage>> & { global?: LoginFlowMessage }}
	 */
...
export type NativeLoginFlow = {
	 ...
	 */
	startSession(params?: NativeParams): Promise<LoginFlowState | void>;
...
export type NativeFlow = Omit<SDK<NativeParams, Promise<EntryResponse>>, 'login' | 'register'> & {
	 ...
	 * @returns {Promise<LoginFlowState | void>} A promise that resolves to the login flow state after starting the session, or void if the session is finalized.
	 */
	startSession(loginParams?: NativeParams): Promise<LoginFlowState | void>;
