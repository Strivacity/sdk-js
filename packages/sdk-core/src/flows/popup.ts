...
export async function createPopupFlow(opts: SDKInitConfig): Promise<PopupFlow> {
	if (!opts.urlHandler) {
		opts.urlHandler = popupUrlHandler;
	}
	if (!opts.callbackHandler) {
		opts.callbackHandler = popupCallbackHandler;
	}
