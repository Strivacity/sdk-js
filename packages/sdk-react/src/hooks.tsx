...
export function useNativeLogin(options: UseNativeLoginOptions = {}): ReturnType<typeof useNativeLoginContext> {
	...
	const stateRef = useRef<Partial<NativeFlowState>>({});

	const [loading, setLoading] = useState<boolean>(true);
	const [forms, setForms] = useState<Record<string, Record<string, unknown>>>({});
	const [messages, setMessages] = useState<Record<string, Record<string, NativeFlowMessage>>>({});
	const [state, setState] = useState<Partial<NativeFlowState>>({});
	...
	const setMessage = useCallback((formId: string, widgetId: string, value: NativeFlowMessage) => {
		setMessages((previousMessages) => ({
			...previousMessages,
			[formId]: {
 ...
 *
 * @param {ComponentType<P>} WrappedComponent - The component to guard.
 ...
 */
export function withAuthGuard<P extends object>(WrappedComponent: ComponentType<P>, options?: WithAuthGuardOptions): ComponentType<P> {
