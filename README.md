...
Strivacity SDK for JavaScript based Applications.
...
```js
import { StrivacityClient } from '@strivacity/sdk-js';
...
```js
client.anonymousIdentity
	.getIdentityById('testId', 'session')
	.then((response) => console.log('success', response))
	.catch((error) => console.log('error', error));
```
...
```js
client.anonymousIdentity.consent
	.getConsents('testId', 'session')
	.then((response) => console.log('success', response))
	.catch((error) => console.log('error', error));
```
